...
export function useSolanaValidators(currency: CryptoCurrency): ValidatorsAppValidator[] {
  ...
  useEffect(() => {
    ...
    getSolanaValidators(currencyId)
      ...
      .catch(() => {
        // drop the memo too, otherwise the next mount would seed the state with a
        // stale list before blanking it again. the cache evicts rejections, so the
        // next mount retries.
        lastSeenValidators.delete(currencyId);
