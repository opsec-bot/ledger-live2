...
# Usage: scan-prs.sh -p '<extended-regex>' [-b develop] [-o outdir] [-j 8] [-l 300] [-x '<exclude-path-regex>']
set -uo pipefail
...
while getopts "p:b:o:j:l:x:" opt; do
  case "$opt" in
    p) PATTERN="$OPTARG" ;;
    b) BASE="$OPTARG" ;;
    o) OUT="$OPTARG" ;;
...
echo >&2
echo "artifacts: $OUT/prs.json  $OUT/hits.tsv  $OUT/diffs/" >&2
